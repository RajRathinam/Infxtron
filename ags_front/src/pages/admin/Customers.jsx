...
import { Search, Trash2, Eye, User, Phone, Mail, MapPin, MessageCircle, Gift, X, Download, Filter, MessageSquare } from "lucide-react";
...
const Customers = () => {
  ...
  // Get customers with feedback (no address but with message)
  const getCustomersWithFeedback = (customerList) => {
    return customerList.filter(customer => {
      const hasMessage = customer.message && customer.message.trim() !== "";
      const hasNoAddress = !customer.address || customer.address.trim() === "";
      return hasMessage && hasNoAddress;
    });
  };
  ...
  useEffect(() => {
    ...
    if (filterFeedback) {
      results = getCustomersWithFeedback(results);
    }

    setFiltered(results);
  }, [search, filterEmail, filterDuplicates, filterFeedback, customers]);
  ...
  const exportToExcel = () => {
    ...
    try {
      ...
      const excelData = filtered.map(customer => ({
        ...
        "Has Message": customer.message && customer.message.trim() !== "" ? "Yes" : "No",
  ...
  const getStats = () => {
    ...
    const withFeedback = getCustomersWithFeedback(customers).length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          <div className="text-2xl font-bold text-amber-600">{stats.withFeedback}</div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-amber-50 px-3 py-2 rounded-lg border border-amber-200 flex-shrink-0">
            <input
              type="checkbox"
              id="filterFeedback"
              checked={filterFeedback}
              onChange={() => setFilterFeedback((prev) => !prev)}
              className="accent-amber-500 w-4 h-4"
            />
            <label htmlFor="filterFeedback" className="text-gray-700 text-xs font-medium whitespace-nowrap">
          ...
          <div className="text-xs text-gray-500 bg-blue-50 px-2 py-1 rounded border border-blue-200 flex-shrink-0">
            ...
            {filterFeedback && ` (${stats.withFeedback} with feedback)`}
          </div>
        </div>
      </div>
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                        ...
                        <User className={filterFeedback ? 'text-amber-600' : 'text-green-600'} size={18} />
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4">
                    {c.address ? (
                      ...
                      <div className="flex items-center gap-2">
                        <MapPin className="text-gray-300" size={14} />
                        <span className="text-gray-400 text-xs">No address provided</span>
                      </div>
                    )}
                  </td>
                    ...
                    <div className="flex justify-center items-center gap-3">
                      <motion.button
                        ...
                        className={`p-2 rounded-lg hover:bg-blue-100 transition-colors duration-200 ${
                          filterFeedback ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'
                        }`}
                        title="View Details"
                      >
                        <Eye size={18} />
                ...
                <td colSpan="5" className="text-center py-12">
                  <div className="flex flex-col items-center justify-center text-gray-400">
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      {search || filterEmail || filterDuplicates || filterFeedback 
          ...
          >
            <motion.div
              ...
              className={`bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden ${
                filterFeedback ? 'border-l-4 border-amber-500' : ''
              }`}
            ...
            >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="opacity-90 text-sm">
                        {filterFeedback ? 'Customer Feedback' : 'Customer Details'}
                      </p>
                    </div>
                  </div>
                  <motion.button
              ...
              <div className="p-6 space-y-4 max-h-96 overflow-y-auto">
                ...
                {/* Message - Highlighted for Feedback */}
                {selectedCustomer.message && (
                  <div className="space-y-3">
                    <h3 className="font-semibold text-gray-800 flex items-center gap-2">
                      <MessageSquare size={16} className={filterFeedback ? "text-amber-500" : "text-indigo-500"} />
                      {filterFeedback ? 'Customer Feedback' : 'Customer Message'}
                    </h3>
                    <div className={`p-3 rounded-lg ${
                      filterFeedback ? 'bg-amber-50 border-l-4 border-amber-400' : 'bg-indigo-50'
                    }`}>
